...
const LOGS_API = "https://script.google.com/macros/s/AKfycbwYObhuqrhtObdOLzHdR7LgGP3iozVXQZEiFPL6Plbt5EJK4q9sfK-Uwvw0jtti_d-K/exec";
...
function renderTable(rows) {
    ...
    logsTableBody.innerHTML = rows.map(row => `
        <tr>
            ...
            <td class="td-time" title="${escHtml(formatTimeFull(row.timestamp))}">${formatTime(row.timestamp)}</td>
            <td>${renderStatusBadge(row.status)}</td>
            <td class="td-price">${formatPrice(row.totalPrice)}</td>
        </tr>
    `).join('');
}

// =============================================
//  Price formatting — peso, 2 decimals, blank if not applicable
// =============================================
...
function formatTime(ts) {
    ...
    h = h % 12 || 12;
    // Short form for the column itself — weekday/year rarely matter at a
    // glance and were the widest part of this cell. Full date still lives
    // in the title attribute for anyone who hovers.
    return `${months[d.getMonth()]} ${d.getDate()} · ${h}:${m} ${ampm}`;
}

// Full weekday/date/year string, used as the <td title="..."> tooltip
// so nothing is actually lost by shortening the visible text above.
