...
const GOOGLE_SHEET_API = "https://script.google.com/macros/s/AKfycbwYObhuqrhtObdOLzHdR7LgGP3iozVXQZEiFPL6Plbt5EJK4q9sfK-Uwvw0jtti_d-K/exec";
...
const navRateBtn           = document.getElementById('navRateBtn');
...
function openModal(item) {
    ...
    // Action button — jumps to the request page with this item pre-filled.
    // Hidden entirely when there's nothing to give out.
    const actionBtn = document.getElementById('modalActionBtn');
    if (actionBtn) {
        if (item.quantity > 0) {
            const isSellable = item.itemType === 'Sellable';
            const reqType    = isSellable ? 'Buy' : 'Borrow';
            actionBtn.textContent = isSellable ? '🛒 Buy This' : '🔄 Borrow This';
            actionBtn.classList.remove('hidden');
            actionBtn.onclick = () => {
                window.location.href = `request.html?item=${encodeURIComponent(item.itemName)}&type=${encodeURIComponent(reqType)}`;
            };
        } else {
...
// Review stats (the footer strip) live in nav.js, which every page loads —
// fetchReviewStats() below is that shared function, called again here after
// a submit so the footer updates without a refresh.
...
navRateBtn.addEventListener('click', () => {
    if (sessionStorage.getItem(REVIEW_SESSION_KEY)) return; // already reviewed this visit
    openReviewModal();
});
...
function lockReviewButton() {
    ...
    navRateBtn.disabled   = true;
    navRateBtn.innerHTML  = '<span class="nav-menu-icon">✓</span>Rated';
}
...
window.addEventListener('load', () => {
    ...
    // Arrived here via the Rate Us link on another page — open the modal
    // directly instead of making them find the button on their own.
    if (new URLSearchParams(location.search).get('rate') === '1' && !sessionStorage.getItem(REVIEW_SESSION_KEY)) {
        openReviewModal();
